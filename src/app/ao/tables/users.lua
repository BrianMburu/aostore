...
function HandleSignUpReward(userId, userCount, currentTime)
    local gift
    local transactionType = "Successful Sign Up"
    ...
    if userCount > 0 and userCount % MILESTONE_INTERVAL == 0 then
     ...
     -- Log transaction
    LogTransaction(userId, transactionType, gift, currentTime)
...
Handlers.add(
    ...
    function(m)
        ...
        HandleSignUpReward(userId, userCount, currentTime)
    end
)
